...
const ScanTicket = () => {
  ...
  const sacanningRef = useRef(false);
  const timeroutRef = useRef<number | null>(null);
  const highlightCodeOnCanvas = (detectedCode: any, ctx: any) => {
    detectedCode.forEach((detectedCode: any) => {
  ...
  const handScan = async (result: any) => {
    ...
    timeroutRef.current = window.setTimeout(() => {
      sacanningRef.current = false;
        ...
        {!data && (
          <div className="bg-gray-200/50 p-6 rounded-lg">
            ...
            <div className="mt-4 flex flex-col gap-4">
              ...
              <p className="flex items-center gap-2 text-slate-700">
                <span className="bg-green-500/50 text-green-700 font-semibold h-8 w-8 flex rounded-full items-center justify-center"></span>
                ...
                <div className="md:col-span-2 space-y-2">
                  ...
                  <div className="rounded-xl p-5">
                    ...
                    <div className="space-y-3 text-slate-600">
                      <p>
                        <span className="font-medium">Điếm xuất phát:</span>{" "}
                        {data?.pickupPoint}
                      </p>

                      <p>
                        <span className="font-medium">Điếm Trả:</span>{" "}
                        {data?.dropPoint}
                      </p>
                      <p>
